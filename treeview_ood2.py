...
class SelfTreeView(Treeview):
    ...
    def treeviewDoubleClick(self, event):  # 双击
        for item in self.tree.selection():
            ...
            if self.column_name[0] == "Database":
                ...
                if arrIndex(self.column_name, "table_level") >= 0:
                # try:
                ...
                # else:
                    ...
                    if int(item_text[arrIndex(self.column_name, "table_level")]) == 0 \
                            and str(type(self.master)).split(".")[1].startswith("Frame"):
                        if len(item_text[arrIndex(self.column_name, "filial_name")]) != 0:
                            filial_name = item_text[arrIndex(self.column_name, "filial_name")].split(",")
                            distance = 0
                            for name in filial_name:
                                self.cursor.execute("select * from " + name + " where parent_id = "
                                                    + item_text[arrIndex(self.column_name, "id")])
                                ...
                                if len(data) != 0:
                                    ...
                                    alignstr = '%dx%d+%d+%d' % (
                                        width, height, ((screenwidth - width) / 2)+distance, ((screenheight - height) / 2)+distance)
                                    tl.geometry(alignstr)
                                    distance = distance + 10
                                    SelfTreeView(tl, column, data, self.cursor)
                    elif int(item_text[arrIndex(self.column_name, "table_level")]) != 0:
                        if arrIndex(self.column_name, "url") >= 0:
                            # im = PIL.Image.open("test.jpg")
                            # img = ImageTk.PhotoImage(im)
                            # imLabel = Label(self.root, image=img).pack()  # 显示图片
                            tl = Toplevel(self.root)
                            tl.title("id="+str(item_text[arrIndex(self.column_name, "id")])+"的相关的图片")
                            width = 300
                            height = 180
                            screenwidth = self.root.winfo_screenwidth()
                            screenheight = self.root.winfo_screenheight()
                            alignstr = '%dx%d+%d+%d' % (
                                width, height, ((screenwidth - width) / 2)+20, ((screenheight - height) / 2)+20)
                            tl.geometry(alignstr)
                            # photo = PhotoImage(file=item_text[arrIndex(self.column_name, "url")])
                            # label = Labelframe(tl,image=photo)
                            # label.pack()
                            im = Image.open(item_text[arrIndex(self.column_name, "url")], 'r')
                            img = ImageTk.PhotoImage(im)
                            label = Label(tl, image=img)
                            label.image = img
                            label.pack()  # 显示图片

                        if len(item_text[arrIndex(self.column_name, "parent_name")]) != 0 and self.cursor is not None:
                            self.cursor.execute("select * from " + item_text[arrIndex(self.column_name, "parent_name")] + " where id = " + item_text[arrIndex(self.column_name, "parent_id")])
                            col = self.cursor.description
                            column = []
                            for i in range(len(col)):
                                column.append(col[i][0])
                            ...
                            if len(data) != 0:
                                tl = Toplevel(self.root)
                                tl.title("与" + item_text[arrIndex(self.column_name, "parent_name")] + "关联" + str(self.cursor.rowcount) + "条数据")
                                width = 300
                                height = 180
                                screenwidth = self.root.winfo_screenwidth()
                                screenheight = self.root.winfo_screenheight()
                        ...
                        if len(item_text[arrIndex(self.column_name, "filial_name")]) != 0:
                            ...
                            for name in filial_name:
                                ...
                                if len(data) != 0:
                                    ...
                                    screenheight = self.root.winfo_screenheight()
                                    alignstr = '%dx%d+%d+%d' % (
                                        width, height, ((screenwidth - width) / 2)+10, ((screenheight - height) / 2)+10)
                                    tl.geometry(alignstr)
